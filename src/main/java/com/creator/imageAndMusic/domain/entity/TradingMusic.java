...
public class TradingMusic {
    ...
    @JoinColumn(name = "seller",foreignKey = @ForeignKey(name = "FK_user_TradingMusic_seller",
            foreignKeyDefinition = "FOREIGN KEY (seller) REFERENCES user(username) ON DELETE CASCADE ON UPDATE CASCADE") ) //FK설정\
    User seller ;//판매자유저

    @ManyToOne(fetch = FetchType.LAZY)
    @JoinColumn(name = "buyer",foreignKey = @ForeignKey(name = "FK_user_TradingMusic_buyer",
            foreignKeyDefinition = "FOREIGN KEY (buyer) REFERENCES user(username) ON DELETE CASCADE ON UPDATE CASCADE") ) //FK설정\
    User buyer;//구매자유저

    @ManyToOne(fetch = FetchType.LAZY)
    @JoinColumn(name = "fileid",foreignKey = @ForeignKey(name = "FK_MusicFileInfo_Tading_img",
            foreignKeyDefinition = "FOREIGN KEY (fileid) REFERENCES music_file_info(fileid) ON DELETE CASCADE ON UPDATE CASCADE") ) //FK설정\
    MusicFileInfo fileid;      //이미지 파일(랭킹에 등록되어있는 - 조회수 5회이상, 좋아요 3이상 - 조건셋업은 나중에)
