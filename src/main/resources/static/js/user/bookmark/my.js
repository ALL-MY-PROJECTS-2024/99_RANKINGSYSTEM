...
const cancelEls = document.querySelectorAll('.image-block .cancel>a');
...
const cancelMusicEls = document.querySelectorAll('.music-block .cancel>a');

cancelMusicEls.forEach(el=>{
    el.addEventListener('click',()=>{

        const id = el.getAttribute('data-id');
        axios.get(`/bookmark/del/music/${id}`)
        .then(resp=>{
            console.log(resp);
            alert("즐겨찾기에서 제거했습니다.");
            location.href="/bookmark/my";
